...
def get_project_blueprint(config: ProjectConfig) -> Dict[str, Any]:
    ...
    tree: Dict[str, Any] = {
        ...
        "children": [
            ...
            {"name": "tiger.lock", "type": "file", "desc": "Tiger Framework integrity lock & signature (x010.tech)"},
...
def write_project_boilerplate(config: ProjectConfig) -> Path:
    ...
    # 1b. tiger.lock (Tiger Framework Lightweight Lockfile & Signature)
    import hashlib
    ...
    lock_data = {
        ...
        "signature": {
            ...
            "logo": "frontend/public/logo.png" if config.include_frontend else "assets/logo.png",
            "integrity": f"sha256-{sig_hash[:32]}",
        },
    }
    (root / "tiger.lock").write_text(json.dumps(lock_data, indent=2), encoding="utf-8")
...
async def health_check():
    ...
    return {{"status": "healthy", "service": "{config.project_name}-backend", "framework": "Tiger Framework (x010.tech)"}}
''',
            encoding="utf-8"
        )
